pageextension 80150 "Planview Shipment (Map)" extends "Planview Shipments"
{
    ...
    begin
        ...
        if Shpmnt.FindSet then repeat
            ...
            if "Lane Type" = "Lane Type"::Collection then begin
                Addr.get(Shpmnt."From Address No.");
            RouteDetails."Marker Fill Color" := 'red';
            end else if "Lane Type" = "Lane Type"::Delivery then begin
...
//MapRoute."Marker Radius" := round("Payable Weight (Order)", 1);
            RouteDetails."Marker Text" := 'LM: ' + format("Loading Meters") + ' ' + Addr.Description + ' ' + Addr.Street + ' ' + Addr."Post Code" + ' ' + Addr.City; 
            RouteDetails.Insert;
        until Shpmnt.Next = 0;
